...
applyOperation:
    ...
    mov     eax, 0
    mov     ebx, 0
    mov     eax, [bufferPtr]     ; get current buffer pointer memory pos
    mov     ebx, [eax]           ; get the value of the buffer on its current pointer (eax)
    mov     [op2], ebx           ; load op2 of multiplication y(n-k) samples
    
    call    multiply             ; multiply alpha * y(n-k)
    ...
    add     ebx, 4               ; buffer pointer memory pos ++
    inc     ecx                  ; buffer counter ++
    
    ; check if k has reached to reset buffer pointer and counter
    mov     edx, [k]             ; move k to edx
    cmp     ecx, edx             ; compare buffer counter to k
    jz      resetBuffer
    
lastMove:
    mov     [bufferPtr], ebx     ; move new memory pos on buffer pointer
    mov     [bufferCtr], ecx     ; save new buffer counter
...
; ------
; resetBuffer()
; Resets buffer data when k has been reached
resetBuffer:
    mov     ebx, buffer         ; new buffer memory pos is buffer start
    mov     ecx, 0              ; reset buffer ccunter
...
shiftHighLow:
    ...
    shl     ebx, cl               ; shift ebx (high) by cl (8)
    ...
    mov     cl, 8                 ; move 8 to bl (shift value)
    shr     ebx, cl               ; shift eax (low) by bl (8)
...
; ------
; calculateOneAlpha()
; Calculates 1 - alpha parameter and stores it in oneAlpha
calculateOneAlpha:
    mov     eax, [alpha]          ; get alpha on eax
    mov     ebx, 65280            ; 65280 is -1 on fixed point arithmetic
    
    mov     [op1], eax            ; load op1 (alpha)
    mov     [op2], ebx            ; load op2 (-1)

    call    multiply              ; multiply alpha by -1
    
    mov     eax, [resultMulti]    ; load result (-alpha) on eax
    mov     ebx, 256              ; 256 is 1 in fixed point arithmetic
    add     eax, ebx              ; add -alpha to 1 (256)
    
    mov     [oneAlpha], ax        ; store result on oneAlpha
...
rwAll:
    ...
    mov     eax, buffer           ; store buffer start memory pos on eax
    mov     [bufferPtr], eax      ; move buffer memory pos on buffer pointer pos
    mov     ebx, 0
